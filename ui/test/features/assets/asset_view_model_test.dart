...
void main() {
  ...
  group('AssetViewModel', () {
    ...
    test('importAssets filters non-image files', () async {
      ...
      when(mockStorageService.importAsset(any, targetFilename: anyNamed('targetFilename')))
          .thenAnswer((invocation) async => Asset(
                id: '1',
                filename: 'test.png',
                path: '/path/to/test.png',
                status: AssetStatus.available,
      ...
      verify(mockStorageService.importAsset('/path/to/test.png', targetFilename: 'test.png')).called(1);
      verifyNever(mockStorageService.importAsset('/path/to/test.txt', targetFilename: anyNamed('targetFilename')));
    });
    ...
    test('importAssets handles rename on collision', () async {
      ...
      when(mockStorageService.loadAssets()).thenAnswer((_) async => [existingAsset]);
      await viewModel.refreshAssets();

      final paths = ['/new/test.png'];
      
      when(mockStorageService.generateUniqueFilename('test.png'))
          .thenAnswer((_) async => 'test_1.png');
      when(mockStorageService.importAsset('/new/test.png', targetFilename: 'test_1.png'))
